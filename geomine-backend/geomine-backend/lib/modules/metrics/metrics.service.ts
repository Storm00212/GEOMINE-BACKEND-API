...
// calculations live in Postgres (see supabase/migrations/0002 and 0003 for
// the formulas and which ones are physics vs. heuristic) — this layer just
// gives the rest of the app typed, named functions instead of raw `.rpc()`
// strings and untyped rows scattered around.
...
export async function getReadingStats(
  ...
  const { data, error } = await repo.callGetReadingStats(machineId, parameterId, from, to);
...
export async function getFleetHealthSnapshot(): Promise<GeneratorHealthSnapshot[]> {
  const { data, error } = await repo.selectFleetHealthSnapshot();
  if (error) throw error;
  return data ?? [];
}

export async function getMachineHealthSnapshot(machineId: string): Promise<GeneratorHealthSnapshot | null> {
  const { data, error } = await repo.selectMachineHealthSnapshot(machineId);
...
export async function getOverloadDuration(machineId: string, from?: string, to?: string): Promise<OverloadDuration | null> {
  const { data, error } = await repo.callGetOverloadDuration(machineId, from, to);
  if (error) throw error;
...
export async function getPowerFactorTrend(machineId: string, from?: string, to?: string): Promise<PowerFactorTrend | null> {
  const { data, error } = await repo.callGetPowerFactorTrend(machineId, from, to);
  if (error) throw error;
...
export async function getEstimatedRul(machineId: string): Promise<RulEstimate | null> {
  const { data, error } = await repo.callGetEstimatedRul(machineId);
  if (error) throw error;
...
export async function getFleetSummary(): Promise<FleetSummaryRow[]> {
  const { data, error } = await repo.selectFleetSummary();
  if (error) throw error;
  return data ?? [];
}
...
export async function getSpecificFuelConsumption(machineId: string, from?: string, to?: string): Promise<SpecificFuelConsumption | null> {
  const { data, error } = await repo.callGetSpecificFuelConsumption(machineId, from, to);
  if (error) throw error;
...
export async function getIdleDuration(machineId: string, from?: string, to?: string): Promise<IdleDuration | null> {
  const { data, error } = await repo.callGetIdleDuration(machineId, from, to);
  if (error) throw error;
...
export async function getMaintenanceRecommendation(machineId: string, sampleSize = 10): Promise<MaintenanceRecommendation | null> {
  const { data, error } = await repo.callGetMaintenanceRecommendation(machineId, sampleSize);
  if (error) throw error;
